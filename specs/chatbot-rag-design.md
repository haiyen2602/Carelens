...
| 8 | **[ĐÃ CHỐT 2026-08-08 — Phase 7 eval/]** `NGUONG_VECTOR`/`NGUONG_LEXICAL` trước RRF (mục 4.3) — đo trên `eval/ground_truth.json` (32 câu, đúng thuốc/field_group biết trước) và `eval/out_of_domain.json` (15 câu - 6 thuốc xác nhận không tồn tại trong 3562 bản ghi, 4 tên thật gõ sai nghiêm trọng, 5 văn bản không liên quan). **Phát hiện quan trọng nhất:** ở giá trị cũ (0.5/0.3), **100% câu out-of-domain lọt qua ngưỡng** (BR-7.3 "không có nguồn → từ chối" không hoạt động với bất kỳ trường hợp nào đã test). Đã đổi sang `NGUONG_VECTOR=0.60`, `NGUONG_LEXICAL=0.55` (`src/config.py`) — GT recall còn 81.2% (26/32, đánh đổi thật, không miễn phí), OOD false-accept tổng thể giảm 100%→46.7%, nhưng KHÔNG đều giữa 3 loại: unrelated_text 20%, severe_typo 25%, **nonexistent_drug vẫn 83.3%** — tên thuốc phương Tây giả trong đúng khuôn mẫu câu hỏi thật dường như khớp ngữ nghĩa theo *cấu trúc câu hỏi*, không phải theo thuốc — tune ngưỡng KHÔNG giải quyết được hết loại này, cần thêm lớp phòng vệ khác (xem #12). Ghi chú riêng: lexical (trigram) score bị nhiễu bất ngờ với câu hỏi RẤT NGẮN so với `noi_dung` dài (vd "1 cộng 1 bằng mấy" đạt lexical=0.667, cao hơn nhiều câu true-match GT) — `word_similarity()` tìm đoạn con khớp nhất trong văn bản dài, dễ trùng ngẫu nhiên với câu hỏi ngắn bất kể liên quan hay không. Toàn bộ số liệu + sweep threshold ở `eval/eval_report.json` và lịch sử review phiên làm việc 2026-08-08. | Đã chốt — Architect, dựa trên `eval/` |
...
| 13b | **[Phase 7 chạy 2026-08-08 — phần số (`NGUONG_VECTOR`/`NGUONG_LEXICAL`) đã chốt ở #8, phần đo hallucination CHƯA kết luận được — lý do là 1 phát hiện riêng, không phải chưa đo]** Chạy `eval/run_eval.py` với LLM-judge (gpt-4o-mini) chấm 32 câu GT: lần 1 báo hallucination rate 18.8% (6/32). Verify tay CẢ 6 case bằng cách đọc lại đúng context đã retrieve — **cả 6 đều là judge sai**, không phải model bịa: 3/6 là hành vi #13a đúng ý muốn (model nói rõ "không có trong nguồn" thay vì bịa) mà judge tính nhầm thành không-grounded; 3/6 còn lại câu trả lời khớp gần như nguyên văn với chunk đã retrieve (đối chiếu trực tiếp, không suy đoán) nhưng judge vẫn báo sai. Sửa `_JUDGE_PROMPT` (phân biệt rõ "thiếu thông tin nhưng nói rõ" khác "bịa thông tin") rồi chấm lại - ra CÙNG tỷ lệ 18.8% nhưng với 1 TẬP HỢP CASE KHÁC (3/6 case cũ được sửa đúng, nhưng có 3 case mới xuất hiện) - đối chiếu tay 1 case mới (Fluopas bảo quản, context chỉ 1 câu ngắn rõ ràng) xác nhận judge VẪN sai dù prompt đã sửa. Nguyên nhân gốc: judge gọi qua `get_llm()` dùng chung `settings.llm_temperature=0.7` - không nhất quán giữa các lần chạy cho 1 tác vụ cần xác định đúng/sai rõ ràng. Đã sửa code (`eval/run_eval.py::_get_judge_llm()`, temperature=0 riêng cho judge, không đổi temperature chung dùng cho `generate_answer`) nhưng CHƯA chạy lại lần 3 để xác nhận cải thiện - cần real API cost thêm, nên dừng ở đây chờ quyết định có đáng chạy tiếp không. **Kết luận tạm, dựa trên verify tay 12 case riêng biệt qua 2 lần chạy: 0 trường hợp hallucination thật được xác nhận** - nhưng đây LÀ mẫu nhỏ (chỉ verify case bị flag, không verify toàn bộ 32 câu), không nên báo cáo "đã đo hallucination rate = 0%" như 1 con số đáng tin cậy dùng cho quyết định khác. | Architect — quyết có chạy lại judge (temp=0, chi phí <$0.01) hay để nguyên phát hiện phương pháp luận này |
| 14 | **[precision@1 thấp (40.6%) dù recall@5 cao (87.5%) — phát hiện 2026-08-08, Phase 7]** Đối chiếu trực tiếp qua `hybrid_search()` cho các câu precision miss: kết quả rank-1 LUÔN là 1 field_group KHÁC của ĐÚNG thuốc, chưa từng thấy nhầm sang thuốc khác. Nguyên nhân: `lexical_score` tính từ `ten_thuoc`, giống hệt nhau ở cả 4 chunk của cùng 1 thuốc — lexical không phân biệt được field_group nào, RRF phải dựa hoàn toàn vào tín hiệu vector để xếp hạng field_group đúng lên đầu, không phải lúc nào cũng chính xác. Ít nghiêm trọng hơn số liệu thô gợi ý (không phải "thường trả lời sai thuốc") vì `answer_generation` dùng cả `top_k=5`, không chỉ rank-1 — nhưng đáng cải thiện cho ranking chuẩn hơn. Không chặn gì hiện tại. | Architect, cân nhắc khi tối ưu ranking (vd boost field_group khớp ý định câu hỏi) |
