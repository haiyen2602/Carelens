...
| 13 | **[Nghi vấn grounding — answer_generation có thể dùng kiến thức nền thay vì chỉ context được cấp — phát hiện 2026-08-08, thử tay qua `/api/v1/chat`]** Câu hỏi "Vitamin C dùng để làm gì" chỉ retrieve được chunk `tac_dung_phu` (tác dụng phụ, KHÔNG có `cong_dung` nào trong 5 nguồn) nhưng câu trả lời vẫn mô tả đúng công dụng chung ("hỗ trợ sức khỏe, tăng cường hệ miễn dịch") — nội dung này không có căn cứ trong context được cấp cho prompt, dấu hiệu model dùng kiến thức nền (pretrained) thay vì grounding thuần theo `_ANSWER_PROMPT`'s "chỉ dựa vào thông tin dưới đây". Phần nói về tác dụng phụ (buồn nôn, mệt mỏi, tăng oxalat niệu) thì khớp gần nguyên văn với chunk đã retrieve — chỉ phần công dụng là đáng ngờ. Đây đúng là rủi ro mục 5 (nguồn trích dẫn bắt buộc) muốn ngăn, và nghiêm trọng hơn việc chọn nhầm sản phẩm cùng tên (mục 10 #12) vì không có cách nào phát hiện qua `sources[]` trả về (sources vẫn liệt kê đầy đủ, trông như đã grounding đúng). Không vá ngay — cần **Phase 7 `eval/`** đo có hệ thống (tỷ lệ hallucination đã có trong kế hoạch Phase 7, `build-kickoff-prompt.md` mục 3) thay vì kết luận từ 1 lần thử tay. | Architect — đo qua eval/ trước khi quyết có cần ràng buộc chặt hơn ở prompt hay đổi model không |
