...
| 13b | **[Đo hệ thống mức độ grounding qua eval/ — số liệu, cần Phase 7]** Kỷ luật prompt (#13a) giảm rủi ro nhưng không đo được TỶ LỆ model vẫn lấp khoảng trống bằng kiến thức nền bất chấp ràng buộc prompt (LLM không đảm bảo tuân thủ 100% chỉ vì được yêu cầu) - cần tập câu hỏi có ground truth + đo tỷ lệ hallucination có hệ thống (đã có trong kế hoạch Phase 7, `build-kickoff-prompt.md` mục 3), không kết luận từ 1 lần thử tay. Cũng là nơi tune số `NGUONG_VECTOR`/`NGUONG_LEXICAL`/`k`/`top_k` (mục 10 #1, #8) - khác bản chất với #13a: đây là thiếu số liệu thực nghiệm, không phải thiếu ràng buộc logic. | Architect, đo qua eval/ ở Phase 7 |
