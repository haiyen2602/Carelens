...
|---|---|---|---|---|---|---|---|---|---|
| Natural renderer | `TASK-V2.5-004` / [PR #189](https://github.com/AI20K-Build-Phase-Cohort-3/P-067/pull/189)+[#191](https://github.com/AI20K-Build-Phase-Cohort-3/P-067/pull/191) (CP1, merged); CP2 đang code trên `feature/TASK-V2.5-004-renderer-cp2`, chưa mở PR | Dyo31122005 | `AGENT_V2_5_RENDERER_ENABLED` (off, chưa thêm code trước CP2 này) | — (chưa canary) | Benchmark thật `gpt-5.6-luna` N=30 (CP1) — chi tiết task file mục 1 | **CP2 đang thực hiện** — cơ chế core (ModelRole.RENDERER, response_policy.py, assemble_reply, golden prose-tự-do 4 âm+4 dương, fallback/timeout ở RENDERER, MULTI_MODEL cost audit) đã code+test thật; **còn thiếu:** wiring renderer vào đúng nhóm response-type (V2.5-DESIGN.md mục 7), chưa PR/merge — chi tiết task file mục "Tiến độ CP2 thực" | Chưa | Chưa | Chưa | Dyo31122005 |
| Context/follow-up | `TASK-V2.5-002` / [PR #183](https://github.com/AI20K-Build-Phase-Cohort-3/P-067/pull/183)+[#184](https://github.com/AI20K-Build-Phase-Cohort-3/P-067/pull/184)+[#186](https://github.com/AI20K-Build-Phase-Cohort-3/P-067/pull/186) (merged) | Dyo31122005 | `AGENT_V2_5_FOLLOWUP_ENABLED` (off, chưa set trên Railway — dùng default) | — (chưa canary) | Golden `--deterministic-only` 15/15 + golden V2.5 (flag ON) 1/1 + candidate-vs-baseline (an toàn/grounding/latency/cost), chi tiết ở task file | CP2 PASS đầy đủ 8/8 mục. **CP3: 3/6 đạt, 2/6 kèm ghi chú, 1/6 không đạt (CI kẹt QUEUED trên runner pool `cohort3`)**. **CP4: deploy flag-off đã verify THẬT trên production** (`railway up` thủ công sau khi vá `.railwayignore` — auto-deploy cũng đang kẹt cùng runner pool; health 200, log sạch, traffic thật bình thường, `_run_doctor_takeover_timeout` chạy OK) — chi tiết task file | **Chưa mở** — cần owner xác nhận riêng, không suy ra từ việc deploy đã xong | Chưa | Dyo31122005 |
