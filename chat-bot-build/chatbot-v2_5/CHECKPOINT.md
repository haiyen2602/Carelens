...
| Context/follow-up | `TASK-V2.5-002` / [PR #183](https://github.com/AI20K-Build-Phase-Cohort-3/P-067/pull/183)+[#184](https://github.com/AI20K-Build-Phase-Cohort-3/P-067/pull/184) (merged) | Dyo31122005 | `AGENT_V2_5_FOLLOWUP_ENABLED` (off) | — (chưa canary) | Golden `--deterministic-only` 15/15 + golden V2.5 (flag ON) 1/1 + candidate-vs-baseline (an toàn/grounding/latency/cost), chi tiết ở task file | CP2 PASS đầy đủ 8/8 mục (187 test targeted, build local, telemetry audit) — xem task file. **CP3: 3/6 đạt, 2/6 đạt kèm ghi chú, 1/6 không đạt (CI `lint-and-test`/`golden-smoke` kẹt QUEUED trên runner pool `cohort3` cho cả 2 PR, không tự force chạy được — merge dựa trên bằng chứng local, không phải CI repo xác nhận)** | Chưa mở (flag vẫn off); metric query/stop condition đã định nghĩa ở task file cho CP4 | Chưa | Dyo31122005 |
