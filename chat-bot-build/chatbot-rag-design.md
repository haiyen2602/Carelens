...
| 35 | **[ĐÃ LÀM 2026-08-13 — vòng 4, mục 5]** Thêm `hourly_conversation_summaries`: một summary độc lập cho mỗi `(patient_id, hour_bucket)` đã kết thúc, gồm `summary_text`, `message_count`, `created_at` và `hidden` để tôn trọng thao tác ẩn lịch sử chat. Job mới dùng chung `AsyncIOScheduler` + `SQLAlchemyJobStore`, chạy phút 0 mỗi giờ và chỉ gọi LLM cho bệnh nhân có `chat_messages` không ẩn trong giờ vừa kết thúc; unique constraint làm job idempotent. Summary **không được import hay truyền vào** `get_recent_context()`/`intent_classification`/`answer_generation`; `chat_history_query` ưu tiên đọc summary, fallback về raw messages khi chưa có. Chi phí: tối đa một LLM call cho mỗi bệnh nhân hoạt động trong một giờ. Migration `0022` (đổi từ `0015`
— xem "Phản hồi review PR#37" bên dưới), test bucket/không hoạt động/idempotency/history source đã pass.
**Phản hồi review 2026-08-14:** mục 5.5 kickoff yêu cầu riêng 1 test gọi `intent_classification` thật xác nhận tóm tắt giờ không rò rỉ vào prompt (đúng dạng §9.4 vòng 3) — trước đó thiếu. Đã thêm `tests/test_chat_history_e2e.py::test_hourly_summary_never_appears_in_intent_classification_prompt` (e2e qua `/api/v1/chat` thật, seed summary nội dung riêng biệt, xác nhận `classify_intent()` không nhận được). **Phản hồi review PR#37 (bot `phoenix-mentor`, 2026-08-14):** migration trùng `revision="0015"` với `0015_email_verification.py` đã có sẵn trên `main` (2 nhánh xây song song, không ai biết về nhánh kia) — đổi số theo đúng pattern đã dùng ở fix collision `0020`→`0021` (commit `35aee33`, `fix/migration-revision-collision-0020`): rename file, `revision=0022`, `down_revision=0021` (head thật của `main` sau khi fix đó merge). Xác nhận `alembic heads` chỉ còn 1 head, chạy full chain `0001→0022` trên DB rỗng không lỗi. | Đã làm — migration đã đổi số 0022, đã thêm test e2e còn thiếu, đã hết trùng revision |
