...
duoc ap dung qua choke-point o src/db/base.py (event "connect" tren engine),
...
def test_ef_search_applied_automatically_via_connect_event_no_manual_set():
    ...
    try:
        ...
        assert int(row[0]) == settings.hnsw_ef_search, (
            ...
            "- event 'connect' o src/db/base.py khong ap dung dung, hoac khong chay"
        )
    finally:
        db.close()
...
def test_bluepine_regression_now_found_via_real_search_distinct_drug_candidates():
    ...
    from src.agents.nodes.drug_confirmation_nodes import _search_distinct_drug_candidates
