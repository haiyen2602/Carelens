...
bo `src/*.py` xem co import TRUC TIEP 1 HTTP client (requests/httpx/aiohttp/
urllib.request) nao KHONG nam trong src/egress_allowlist.py hay khong.

Hien tai: repo dung `openai`/`langchain_openai` SDK (goi httpx NOI BO nhu 1
dependency da vet, khong phai code tu viet) - KHONG co file nao trong src/
...
from src.egress_allowlist import ALLOWED_EGRESS_MODULES  # noqa: E402
...
def test_no_direct_network_client_import_outside_allowlist():
    ...
    for py_file in _SRC_ROOT.rglob("*.py"):
        rel_path = py_file.relative_to(_SRC_ROOT.parent).as_posix()
    ...
    assert violations == [], (
        ...
        f"src/egress_allowlist.py::ALLOWED_EGRESS_MODULES: {violations}. "
