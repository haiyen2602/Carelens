...
- **Tích hợp API linh hoạt**: đổi nhà cung cấp chỉ bằng một dòng trong
  `api_key.json` — OpenAI, Anthropic, Google AI Studio, OpenRouter, Groq, Together,
  Ollama, LM Studio.
...
Điền 4 trường vào `api_key.json`:
...
> `provider`: `openai` cho mọi endpoint tương thích OpenAI, `claude` cho SDK
> Anthropic. `base_url` nhận tên viết tắt (`gemini`, `openrouter`, `groq`,
...
| `camera_counter.py` | Vòng lặp webcam, overlay, ghi kết quả (chương trình chính) |
| `analyze_image.py` | Đếm trên file ảnh — dùng để đo độ chính xác và tinh chỉnh prompt |

> `api_key.json` nằm trong `.gitignore`. Đừng commit hoặc chia sẻ file này.
