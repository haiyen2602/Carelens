...
class V2AgentKnowledgeService:
    ...
    def _ranked_catalog_matches(self, query: str) -> list[DrugCatalogItem]:
        ...
        truncates it. The one place both ``search_catalog`` and
        ``search_catalog_unique_match`` (BUILD-45) read from, so a query's
        TRUE candidate count is never re-derived from an already-truncated
        list (which would conflate "the caller only asked for N" with "there
        is genuinely only one match")."""
    ...
    def search_catalog_unique_match(self, query: str) -> DrugCatalogItem | None:
        ...
        if len(query_tokens) == 1 and len(query_tokens[0]) <= 3:
            return None
        strong_matches = [item for item in self.catalog_items if self._name_score(query, item.ten_thuoc) >= self._UNIQUE_MATCH_SCORE_FLOOR]
